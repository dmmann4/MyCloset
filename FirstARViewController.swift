...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    public func renderer(_ renderer: SCNSceneRenderer, nodeFor anchor: ARAnchor) -> SCNNode? {
        ...
        if let imageAnchor = anchor as? ARImageAnchor {
            ...
            if imageAnchor.referenceImage.name == "newJacket" {
                self.showSimpleActionSheet2(controller: self)
            }
            
            if imageAnchor.referenceImage.name == "newShirt" {
    ...
    func showSimpleActionSheet1(controller: UIViewController) {
        let alert = UIAlertController(title: "Cool Black Shirt", message: "Do you want to try this on?", preferredStyle: .actionSheet)
    ...
    func showSimpleActionSheet2(controller: UIViewController) {
        let alert = UIAlertController(title: "That Jacket Would look great on you!", message: "Do you want to try this on?", preferredStyle: .actionSheet)
        alert.addAction(UIAlertAction(title: "Yes!", style: .default, handler: { (_) in
